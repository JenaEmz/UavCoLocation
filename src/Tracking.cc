...
>>>>>>> 8736e0c5a964f99dadb8fac1b24e05251a0ddfa0
        ...
        if (LoopDetecting())
        {
            bool success = RelativePoseEPnp();
            res = mCurrentFrame.mTcw.clone();
            std::cout << "loop detected;"
                      << "relative pose is " << success << std::endl;
        }
        else
            res = cv::Mat();
...
<<<<<<< HEAD
    for (int i = 0; i < mvpMapPoints.size(); i++)
    {
        ...
        cv::imwrite("/home/jena/csq_ws/leftMatch1.png", out_left_1);
        cv::imwrite("/home/jena/csq_ws/leftMatch2.png", out_left_2);
    }
